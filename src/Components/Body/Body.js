...
class body extends React.Component {
    ...
    upgradeAddHandler = (upgradedState, newPointTotal, updatedCards, clickedUpgrade) => {

        let commanderChosen = this.state.commanderChosen;
        console.log(clickedUpgrade);
        ...
        //try setting the true in this loop and false in a different one
        let upgrades = [
            {
                "id": 1076,
                "title": "Admiral Chiraneau",
        ...
        if (clickedUpgrade.title === "Darth Vader") {
            uniqueCards.forEach((upgrade, index) => {
                if (clickedUpgrade.title === upgrade.title) {
                    uniqueCards.splice(index, 1)
                }
            })
        }
